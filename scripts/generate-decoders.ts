...
function parseEnums(content: string): EnumDef[] {
  ...
  const enumRegex = /export const (\w+) = __t\.enum\("(\w+)",\s*\{([^}]+)\}\);/g;
...
async function parseTables(typesContent: string, bindingsDir: string): Promise<TableDef[]> {
...
function generateDecoderFile(enums: EnumDef[], tables: TableDef[]): string {
  ...
  lines.push('  if (typeof micros !== "number" && typeof micros !== "bigint") return value;');
  ...
  lines.push("const optionDecoder = (inner?: Decoder): Decoder => (value) => {");
  lines.push("  if (!Array.isArray(value)) return value;");
  ...
  lines.push("  // Pass the payload directly to inner decoder - it knows how to handle its format");
  lines.push("  if (inner) {");
  lines.push(
  ...
  lines.push("  return Array.isArray(payload) && payload.length === 1 ? payload[0] : payload;");
  lines.push("};");
  lines.push("");

  // Table column decoders
  lines.push("// Table column decoders");
  lines.push("export const TABLE_DECODERS: Record<string, Record<string, Decoder>> = {");
    ...
    for (const col of complexColumns) {
      switch (col.kind) {
        case "enum":
          ...
          break;
        case "option-enum":
          lines.push(`    '${col.sqlName}': optionDecoder(enumDecoders.${col.enumName}),`);
          break;
  ...
  lines.push("  const decoders = tableName ? TABLE_DECODERS[tableName] : undefined;");
...
async function main() {
  ...
  const typesContent = await fs.readFile(path.join(bindingsDir, "types.ts"), "utf-8");
