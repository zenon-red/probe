	...
	async run({ args }) {
		...
		if (forceHelpRequested() || !args.name) {
			printHelp({
				...
				description:
					"Complete required local and Nexus setup for autonomous participation",
				usage: [
			...
			if (!args.name) {
				error("NAME_REQUIRED", "--name is required. Example: probe onboard --name 'Alpha Centauri'");
			}
			return;
		}
		...
		function finish(s: typeof state) {
			...
			const hasManualRequired = s.steps.some(
				(st) => st.status === "manual_required",
			);
				...
				if (schedulerStep) {
					...
					note(
						buildAgenticRuntimePlan({
							...
							intervalMinutes: 30,
							prompt:
								"Run probe next and follow its instructions exactly.",
						}),
