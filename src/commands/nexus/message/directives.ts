...
export const messageDirectivesCommand = defineCommand({
  meta: { name: "directives", description: "List directive messages (all, channel, or project)" },
  args: {
    target: {
      type: "positional",
  ...
  async run({ args }) {
    ...
    try {
      ...
      const result = listMessages(ctx, args.target, "directive", limit, args.context);
      success(result);
    } catch (err) {
