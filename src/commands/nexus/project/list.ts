...
export default defineCommand({
  meta: { name: "list", description: "List projects with optional status filter" },
  args: {
    status: { type: "string", description: "Filter by status: active|paused" },
    limit: { type: "string", description: "Max projects returned" },
  ...
  async run({ args }) {
    ...
    await runWithBoundary(async () => {
      ...
      if (args.status) {
        projects = projects.filter((p) => ProjectStatus.matches(p.status, args.status as string));
      }
