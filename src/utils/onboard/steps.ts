...
export async function resolveIdentity(
	...
	state.passwordFile =
		state.args["password-file"] ||
		join(homedir(), ".probe", "wallets", `${state.walletName}.pass`);
	return true;
}
	...
	if (await walletExists(state.walletName)) {
		...
		if (state.args["dry-run"]) {
			...
			addStep(
				...
				"wallet",
				"pass",
				`Created wallet ${state.walletName} at ${result.address}`,
			);
		}
	}
}
...
export async function registerAgentStep(state: OnboardState): Promise<boolean> {
	...
	try {
			...
			async (ctx) => {
				...
				if (existing) {
					addStep(
						...
						"registration",
						"pass",
						`Agent ${existing.id} already registered`,
					);
					return;
				}
		...
		return !state.steps.some(
			(s) => s.step === "registration" && s.status === "fail",
		);
	} catch (err) {
		addStep(
			state,
...
export async function installSkillsStep(state: OnboardState): Promise<void> {
	...
	addStep(
		state,
		"skills",
		skillsResult.installed ? "pass" : "warn",
		skillsResult.detail,
	);
}
