...
export async function fetchGitHubReleaseByVersion(version: string): Promise<GitHubRelease> {
...
export async function upgradeViaBinary(release: GitHubRelease, _target: string): Promise<void> {
  ...
  const targetPath = resolve(binDir, (platform() as string) === "windows" ? "probe.exe" : "probe");
