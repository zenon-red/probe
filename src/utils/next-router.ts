...
export function chooseNext(ctx: CommandContext, agent: Agent): NextAction {
  ...
  if (personalChannel) {
    const cutoff =
      (agent.lastHeartbeat ? toMicros(agent.lastHeartbeat) - BigInt(3600 * 1_000_000) : 0n) || 0n;
    const recent = messages
      .filter((m) => m.channelId === personalChannel.id && toMicros(m.createdAt) > cutoff)
      .slice(0, 3);
    if (recent.length > 0) {
      return {
        kind: "inbox",
  ...
  if (AgentRole.is.zeno(agent.role)) {
    ...
    const votedIdeaIds = new Set(votes.filter((v) => v.agentId === agent.id).map((v) => v.ideaId));
    const pendingIdeas = ctx
      .iter<Idea>("ideas")
      .filter((i) => IdeaStatus.is.voting(i.status) && !votedIdeaIds.has(i.id))
      .sort((a, b) => {
        const aMicros = toMicros(a.createdAt);
        const bMicros = toMicros(b.createdAt);
        return aMicros < bMicros ? -1 : aMicros > bMicros ? 1 : 0;
    ...
    const owned = tasks.find(
      (t) =>
        ...
        (TaskStatus.is.claimed(t.status) || TaskStatus.is.inProgress(t.status)),
    );
    if (owned) {
      return {
        kind: "continue_task",
  ...
  if (AgentRole.is.zoe(agent.role)) {
    ...
    const needProject = approvedIdeas.find((i) => !projectIdeaIds.has(i.id));
    if (needProject) {
      return {
        kind: "project_setup",
    ...
    const activeProjects = ctx
      .iter<{ id: bigint; status: { tag: string }; sourceIdeaId: bigint }>("projects")
      .filter((p) => p.status.tag === "Active");
    for (const project of activeProjects) {
      const taskCount = ctx.iter<Task>("tasks").filter((t) => t.projectId === project.id).length;
      if (taskCount === 0) {
        return {
          kind: "create_tasks",
    ...
    const reviewTasks = ctx
      .iter<Task>("tasks")
      .filter((t) => TaskStatus.is.review(t.status) && t.reviewCount > 0)
      .slice(0, 3);
