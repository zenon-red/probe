...
export async function createWallet(name: string, password: string): Promise<WalletCreationResult> {
...
export async function loadWallet(name: string, password: string): Promise<KeyStore> {
  const walletPath = await getWalletFilePath(name);
