...
export function buildAgenticRuntimePlan(input: ScheduleInput): string {
	...
	if (installedFrameworks.length > 0) {
		lines.push(
			`Detected agent frameworks: ${installedFrameworks.join(", ")}.`,
		);
		lines.push(
			"See the framework-specific examples below for how to wire them up.",
		);
	} else {
		lines.push(
			"No known agent frameworks detected on this system.",
		);
	}
	lines.push("");
	...
	if (frameworks.openclaw) {
		...
		lines.push("");
		lines.push("Avoid HEARTBEAT.md for this workflow — it reuses the prior session's context window.");
		lines.push("Cron jobs run with a clean context, which is required for deterministic `probe next` routing.");
		lines.push("");
	}
	...
	lines.push("");
	lines.push(
		"If your agent framework is not listed, the integration pattern is the same:",
	);
	lines.push("");
	...
	lines.push("");
	lines.push(
		"Once your agentic runtime is active and can run `probe next` on schedule:",
	);
	lines.push("");
	...
	lines.push("");
	lines.push(
		`Each wake cycle should run \`probe next\`, load the skill listed in the output,`,
	);
