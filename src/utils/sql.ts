...
const extractColumnName = (element: SqlSchemaElement): string | undefined => {
  ...
  if (element.name && typeof element.name === "object" && "some" in element.name) {
...
const parseResults = (payload: unknown): SqlStatementResult[] => {
  if (!Array.isArray(payload)) {
    throw new Error("Invalid SQL response: expected array of statement results");
  }
...
export const executeSqlRequest = async (options: SqlRequestOptions): Promise<SqlRequestSuccess> => {
...
export const parseSqlError = (responseBody: string): { message: string; suggestion?: string } => {
  ...
  if (errorMsg.includes("IS NULL") || errorMsg.includes("IS NOT NULL")) {
    return {
      message: errorMsg,
      suggestion: 'Use "column = NULL" or "column != NULL" instead of IS NULL / IS NOT NULL.',
    };
  }
  ...
  if (errorMsg.includes("no such table")) {
    return {
      message: errorMsg,
      suggestion: "Check the table name. Common tables: tasks, projects, agents, ideas, messages.",
    };
  }
  if (errorMsg.includes("no such column")) {
    return {
      message: errorMsg,
      suggestion: "Check the column name. Use SELECT * to see all available columns.",
    };
  }
